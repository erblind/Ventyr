...
const VerticalSnapEventCard: React.FC<{ 
  ...
  return (
  <div className="h-full w-full snap-start relative flex-shrink-0 bg-dark-bg overflow-hidden">
     {/* Immersive Background with slow zoom animation */}
     <div className="absolute inset-0 overflow-hidden">
         <img 
            src={event.image} 
            className="w-full h-full object-cover animate-[zoom_20s_infinite_alternate]" 
            style={{ animation: 'zoom 20s ease-in-out infinite alternate' }}
            alt={event.title}
         />
         <style>{`
            @keyframes zoom {
     ...
     {/* Header Area */}
     <div className="absolute top-0 left-0 right-0 p-6 pt-24 z-20 flex justify-between items-start w-full bg-gradient-to-b from-black/60 to-transparent">
         {/* Organizer Pill */}
         <div className="flex items-center gap-2 bg-white/10 backdrop-blur-xl px-2 pr-4 py-1.5 rounded-full border border-white/10 shadow-lg cursor-pointer hover:bg-white/20 transition-all" onClick={() => onFollow(event.organizer.id)}>
             <img src={event.organizer.avatar} className="w-8 h-8 rounded-full border border-white/20 object-cover"/>
             <div className="flex flex-col">
                <span className="text-[10px] text-gray-300 font-medium leading-none mb-0.5">Presented by</span>
                <span className="text-xs font-bold text-white leading-none">{event.organizer.name}</span>
             </div>
         </div>
         
         {/* Live/Status Badge */}
         <div className="flex gap-2">
            <div className="bg-black/40 backdrop-blur-md border border-white/10 text-white text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1.5">
                <div className={`w-1.5 h-1.5 rounded-full ${event.status === 'published' ? 'bg-neon-green animate-pulse' : 'bg-yellow-500'}`}></div>
                {event.status === 'published' ? 'Live' : 'Upcoming'}
            </div>
         </div>
     </div>

     {/* Right Side Interactions (Social Stack) */}
     <div className="absolute right-4 bottom-32 z-20 flex flex-col gap-5 items-center pb-2">
         {[
             { icon: Heart, count: event.likes, action: onLike, active: isLiked, color: 'text-neon-pink' },
             { icon: MessageCircle, count: event.comments.length, action: () => setShowComments(true), active: false, color: 'text-white' },
             { icon: Share2, label: 'Share', action: () => addToast({type: 'info', message: 'Link copied!'}), active: false, color: 'text-white' }
         ].map((item, idx) => (
             <div key={idx} className="flex flex-col items-center gap-1 group">
                <button 
                    onClick={item.action}
                    className={`w-12 h-12 rounded-full flex items-center justify-center border backdrop-blur-xl transition-all duration-200 active:scale-90 shadow-lg ${item.active ? 'bg-red-500/20 border-red-500 text-red-500' : 'bg-black/20 border-white/10 text-white hover:bg-white/10 hover:scale-105'}`}
                >
                    <item.icon className={`w-6 h-6 ${item.active ? 'fill-current' : ''}`} />
                </button>
                <span className="text-[11px] font-bold text-white drop-shadow-md">{item.count !== undefined ? item.count : item.label}</span>
             </div>
         ))}
     </div>

     {/* Bottom Content Area */}
     <div className="absolute bottom-0 left-0 right-0 z-20 p-6 pb-24 flex flex-col items-start gap-4">
         <div className="max-w-[80%] space-y-2 animate-in slide-in-from-bottom-5 duration-500">
             <div className="flex items-center gap-2 mb-1">
                 {event.tags.map(tag => (
                     <Badge key={tag} color="blue" className="bg-neon-blue/20 text-neon-blue border-neon-blue/30 backdrop-blur-md shadow-[0_0_10px_rgba(75,107,255,0.3)]">#{tag}</Badge>
                 ))}
             </div>
             
             <h2 className="text-4xl font-black text-white leading-[0.95] tracking-tight drop-shadow-xl font-sans" style={{ textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
                 {event.title}
             </h2>
             
             <div className="flex items-center gap-4 text-sm font-medium text-gray-200 mt-2">
                <div className="flex items-center gap-1.5 bg-black/30 px-2 py-1 rounded-lg backdrop-blur-sm border border-white/5">
                    <Calendar className="w-4 h-4 text-neon-pink"/> {event.date}
                </div>
                <div className="flex items-center gap-1.5 bg-black/30 px-2 py-1 rounded-lg backdrop-blur-sm border border-white/5">
                    <MapPin className="w-4 h-4 text-neon-blue"/> {event.location}
                </div>
             </div>
         </div>

         {/* Call to Action Bar */}
         <div className="w-full flex items-center gap-3 mt-2 pr-16 animate-in slide-in-from-bottom-8 duration-700">
             <Button 
                 variant="primary" 
                 onClick={() => onOpen(event)}
                 className="flex-1 h-14 rounded-2xl text-base font-bold shadow-[0_0_30px_rgba(75,107,255,0.4)] border border-white/20 bg-gradient-to-r from-neon-blue to-neon-purple hover:brightness-110 active:scale-95 transition-all"
             >
                 Get Tickets
             </Button>
             <div className="h-14 px-5 flex flex-col items-center justify-center rounded-2xl bg-white/10 backdrop-blur-xl border border-white/10">
                 <span className="text-[10px] text-gray-300 font-bold uppercase tracking-wider">Starting</span>
     ...
     {showComments && (
         <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex flex-col justify-end animate-in slide-in-from-bottom-10 fade-in duration-200">
             <div className="bg-[#0F1316] rounded-t-[32px] border-t border-white/10 h-[70%] flex flex-col shadow-2xl relative overflow-hidden">
                 ...
                 <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                     <h3 className="font-bold text-white text-lg">Comments <span className="text-neon-blue">({event.comments.length})</span></h3>
                     <button onClick={() => setShowComments(false)} className="p-2 rounded-full bg-white/5 hover:bg-white/10 transition-colors"><XIcon className="w-5 h-5 text-gray-300"/></button>
                 </div>
                 <div className="flex-1 overflow-y-auto p-6 space-y-6">
                     {event.comments.map(c => (
                         <div key={c.id} className="flex gap-4 group">
                             <div className="relative">
                                <img src={c.avatar} className="w-10 h-10 rounded-full border border-white/10 object-cover"/>
                                <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-neon-green rounded-full border-2 border-[#0F1316]"></div>
                             </div>
                             <div className="flex-1">
                                 <div className="flex justify-between items-baseline mb-1">
                                    <p className="text-sm text-white font-bold">{c.user}</p>
                                    <span className="text-[10px] text-gray-500">{c.timestamp}</span>
                                 </div>
                                 <p className="text-sm text-gray-300 leading-relaxed bg-white/5 p-3 rounded-r-xl rounded-bl-xl border border-white/5 group-hover:border-white/10 transition-colors">{c.text}</p>
                             </div>
                         </div>
                     ))}
                 ...
                 <div className="p-4 bg-black/20 backdrop-blur-lg border-t border-white/5 pb-8">
                     <div className="relative">
                         <Input placeholder="Add a comment..." className="!mb-0 h-12 text-sm pl-4 pr-12 bg-white/5 border-white/10 rounded-full focus:border-neon-blue/50" />
                         <button className="absolute right-2 top-2 p-2 bg-neon-blue text-white rounded-full hover:bg-neon-blue/80 transition-colors">
...
const App: React.FC = () => {
  ...
  const NavBar = () => {
    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 bg-black/60 backdrop-blur-xl border-t border-white/5 p-4 flex justify-around items-center z-40 pb-8 md:pb-4 shadow-2xl">
        <button onClick={() => setViewState(ViewState.DISCOVER)} className={`flex flex-col items-center gap-1 transition-all ${viewState === ViewState.DISCOVER ? 'text-neon-blue scale-110' : 'text-gray-500 hover:text-white'}`}>
          <LayoutGrid className="w-6 h-6" />
          <span className="text-[10px] font-bold">Discover</span>
        </button>
         <button onClick={() => setViewState(ViewState.RESELL_MARKET)} className={`flex flex-col items-center gap-1 transition-all ${viewState === ViewState.RESELL_MARKET ? 'text-neon-blue scale-110' : 'text-gray-500 hover:text-white'}`}>
          <ShoppingBag className="w-6 h-6" />
          <span className="text-[10px] font-bold">Market</span>
        </button>
        <button onClick={() => setViewState(ViewState.MY_TICKETS)} className={`flex flex-col items-center gap-1 transition-all ${viewState === ViewState.MY_TICKETS ? 'text-neon-blue scale-110' : 'text-gray-500 hover:text-white'}`}>
          <TicketIcon className="w-6 h-6" />
          <span className="text-[10px] font-bold">Tickets</span>
        </button>
        <button onClick={() => setViewState(ViewState.CHAT_HUB)} className={`flex flex-col items-center gap-1 transition-all ${viewState === ViewState.CHAT_HUB ? 'text-neon-blue scale-110' : 'text-gray-500 hover:text-white'}`}>
          <MessageSquare className="w-6 h-6" />
          <span className="text-[10px] font-bold">Chat</span>
        </button>
        <button onClick={() => setViewState(ViewState.PROFILE)} className={`flex flex-col items-center gap-1 transition-all ${viewState === ViewState.PROFILE ? 'text-neon-blue scale-110' : 'text-gray-500 hover:text-white'}`}>
        ...
        <>
          <div className="h-screen w-full pb-20 overflow-hidden relative">
            ...
            {viewState === ViewState.DISCOVER && (
                <>
                    <h1 className="absolute top-6 left-0 right-0 text-center text-white/90 font-black tracking-widest text-xl z-30 drop-shadow-md mix-blend-overlay opacity-50 pointer-events-none">VENTYR</h1>
                    <div className="absolute top-0 right-0 p-4 z-40 pt-12">
                        <button 
                            onClick={() => setViewState(ViewState.ORGANIZER_DASHBOARD)}
                            className="p-3 bg-black/20 backdrop-blur-xl border border-white/10 rounded-full text-white hover:bg-white/10 transition-colors shadow-lg"
                        >
